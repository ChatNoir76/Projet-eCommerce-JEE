	...
	<section id='contenu' class='contenu'>
		<section id='cadreBlanc' class='cadreBlanc'>
			<c:if test="${empty sessionScope.client || sessionScope.clientStatut < 2}">
				<p>Mon panier - commande n°<c:out value="${ panier.get_idCommande() }"></c:out></p>
			</c:if>
			<c:if test="${empty sessionScope.client || sessionScope.clientStatut >= 2}">
				<p>Le panier de <c:out value="${ client.get_nom() }"></c:out> (<c:out value="${ client.get_mail() }"></c:out>) - commande n°<c:out value="${ panier.get_idCommande() }"></c:out></p>
			</c:if>
			  ...
			  <tbody>
  				<c:forEach var="lc" items="${ panier.articles }">
					 <tr>
						...
						<td><span class="badge badge-secondary badge-pill"><input type="number" min="0" onChange="modificationQuantityPanier(${ lc.get_produit().get_id() }, this.value)" value="<c:out value="${ lc.get_quantite() }" ></c:out>" style="color: white;background-color: grey;width: 50px" /></span></td>
				...
				<tr>
			      ...
			      <td></td>
			      <td></td>
			      <td><strong><span class="text-warning">Remise Panier</span></strong></td>
			      <td>
			      	<strong><span class="text-warning">
						<fmt:formatNumber type = "number" minFractionDigits = "2" value = "${ panier.get_commande().get_remiseGlobale() * 100}"/>%
					</span></strong>
			      </td>
			    </tr>
			    <tr>
			...
			<div style="display: flex;flex-direction: center;">
				<c:if test="${empty sessionScope.client || sessionScope.clientStatut >= 2}">
					...
					<input type="text" id="codereduction" value="" maxlength="20" placeholder="code de reduction" />
					<button type="button" style="margin-right: 35px;" class="btn btn-secondary" onClick="codereduction()">ok</button>
				</c:if>
				<button type="button" style="margin-right: 35px;" class="btn btn-info" onClick="refreshPanier()">Mettre à jour mon panier</button>
				<c:if test="${empty sessionScope.client || sessionScope.clientStatut < 2}">
					<button type="button" class="btn btn-danger" onClick="document.location.href='/eCommerce/connexion'">se connecter</button>
				</c:if>
				<c:if test="${empty sessionScope.client || sessionScope.clientStatut >= 2}">
					<button type="button" class="btn btn-success" onClick="refreshPanier()">valider mon panier</button>
				</c:if>
			</div>
		</section>
	</section>
